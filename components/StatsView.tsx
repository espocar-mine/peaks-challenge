...
export const StatsView: React.FC<Props> = ({ plans, currentWeekId, onSelectWeek }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar 
              dataKey="hours" 
              radius={[4, 4, 4, 4]}
              onClick={(data) => onSelectWeek(Number(data.name))}
      ...
      <div className="flex justify-between items-center text-xs text-slate-400 mt-6 px-3 border-t border-slate-100 pt-4">
        <span>Week 1</span>
        <span>Avg {data.reduce((acc, curr) => acc + curr.hours, 0) / data.length < 10 ? 'Low' : 'High'} Vol</span>
        <span>Week 16</span>
      </div>
    </div>
  );
